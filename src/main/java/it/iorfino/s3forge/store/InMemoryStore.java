...
public final class InMemoryStore implements MultipartStore {
     ...
     * {@inheritDoc}
     *
     * <p>Stores the object payload as a defensive copy of the request bytes,
     * alongside its metadata (ETag, content type, CRC32 checksum). This allows
     * subsequent {@link #getObject} calls to return independent streams over
     * the same immutable bytes.</p>
     */
    @Override
    public void putObject(String bucket, String key,
        ...
        b.put(key, bytes);
        metadata.get(bucket).put(key, new StoredObject(
            bucket, key, bytes.length, etag, contentType,
            Instant.now(), checksumCrc32, null));
    }

    /**
     * {@inheritDoc}
     *
     * <p>Returns a fresh {@link ByteArrayInputStream} over the stored bytes on
     * every call, so that concurrent readers do not interfere with each
     * other.</p>
     */
    @Override
    public Optional<StoredObject> getObject(String bucket, String key) {
        ...
        return Optional.of(new StoredObject(
            ...
            meta.etag(), meta.contentType(), meta.lastModified(),
            meta.checksumCrc32(),
     ...
     * {@inheritDoc}
     *
     * <p>Iterates the keyspace in lexicographic order using a {@link TreeMap}
     * snapshot, so that concurrent writes do not disturb an in-progress
     * listing.</p>
     */
    @Override
     ...
     * {@inheritDoc}
     *
     * <p>Generates a fresh upload id using {@link java.util.UUID} and records
     * the upload in an internal map keyed by that id.</p>
     */
    @Override
     ...
     * {@inheritDoc}
     *
     * <p>Concatenates the bytes of each requested part in order. The final
     * ETag follows the S3 multipart convention: the MD5 of the concatenation
     * of the parts' binary MD5 digests, suffixed with
     * {@code -<partCount>}.</p>
     */
    @Override
