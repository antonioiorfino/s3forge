 ...
 *
 * <p>Each constant bundles the AWS error code, a human-readable message and the corresponding HTTP
 * status code, so handlers can emit a well-formed {@code <Error>} XML document with a single call.
 *
 * @since 0.1.0
 */
public enum S3Error {
    ...
    BUCKET_NOT_EMPTY("BucketNotEmpty", "The bucket you tried to delete is not empty.", 409),

    /** The bucket name does not conform to S3 naming rules. */
    INVALID_BUCKET_NAME("InvalidBucketName", "The specified bucket is not valid.", 400),

    /** The Content-MD5 header did not match the computed digest. */
    BAD_DIGEST("BadDigest", "The Content-MD5 you specified did not match what we received.", 400),

    /** A request parameter is malformed or missing. */
    INVALID_ARGUMENT("InvalidArgument", "Invalid argument.", 400),

    /** The copy source and destination are identical and no REPLACE directive was given. */
    INVALID_REQUEST_COPY_SELF(
            "InvalidRequest",
    ...
    INVALID_RANGE("InvalidRange", "The requested range is not satisfiable", 416),
    ...
    INTERNAL_ERROR("InternalError", "We encountered an internal error.", 500);
