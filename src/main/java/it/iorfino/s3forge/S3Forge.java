 ...
 *
 * <ul>
 *   <li>{@link InMemoryStore} — everything is kept in RAM and lost on shutdown.
 *   <li>{@link FileSystemStore} — buckets are mapped to directories under a configurable root path,
 ...
 * }</pre>
 *
 * <p><strong>Thread safety:</strong> instances of this class are safe to start and stop from
 * multiple threads, but {@link #start()} must be called at most once per instance.
 *
 * @since 0.1.0
 */
public final class S3Forge implements AutoCloseable {
    ...
    /**
     * Creates a new instance. Use {@link #builder()} instead of calling this constructor directly.
     *
     ...
     *
     * <p>If the configured port is {@code 0}, the operating system assigns an ephemeral port. Use
     * {@link #port()} after this method returns to learn the actual bound port.
     *
     ...
     *
     * <p>This method is idempotent: calling it on an already-stopped instance is a no-op.
     */
    public void stop() {
        server.stop();
    }
     ...
     *
     * <p>Equivalent to calling {@link #stop()}. Provided so that S3Forge can be used in
     ...
     *
     * <p>If {@link Builder#port(int)} was called with {@code 0}, this method returns the ephemeral
     * port chosen by the OS. It is only meaningful after {@link #start()} has returned
     * successfully.
     *
     ...
     *
     * <p>A storage backend <strong>must</strong> be configured before calling {@link #build()}:
     * either {@link #inMemory()} or {@link #fileSystem(Path)}. Failing to do so results in an
    ...
    public static final class Builder {
         ...
         *
         * <p>Use {@code 0} to let the operating system pick an ephemeral port; retrieve the actual
         * port via {@link S3Forge#port()} after startup. Default is {@code 8001}.
         *
         ...
         *
         * <p>Data is lost when the server is stopped. Suitable for fast, isolated unit tests.
         *
         * @return this builder, for chaining
         */
        ...
        /**
         * Configures the server to persist bucket and object data on the local filesystem, under
         * the given root directory.
         *
         * <p>Each bucket is represented as a sub-directory of {@code rootDir}, and each object as a
         * file inside it. The directory is created if it does not exist.
         *
         * @param rootDir the root directory for persistence; must not be {@code null}
         * @return this builder, for chaining
         */
         ...
         *
         * <p>When enabled, a request whose {@code Host} header is {@code <bucket>.<domain>} is
         * interpreted as targeting the bucket {@code <bucket>}, and the request path is treated as
         * relative to that bucket. Requests whose {@code Host} is exactly the base domain (or an IP
         * address) continue to use path-style addressing.
         *
         * <p>Typical usage in tests: {@code .virtualHostDomain("localhost")} combined with a client
         * configured to resolve {@code <bucket>.localhost}.
         *
         * @param domain the base domain, e.g. {@code "localhost"} or {@code "s3.test"}; must not be
         *     {@code null} or empty
         * @return this builder, for chaining
        ...
        public Builder virtualHostDomain(String domain) {
            if (domain == null || domain.isBlank()) {
                throw new IllegalArgumentException("virtualHostDomain must not be null or empty");
            }
         ...
         *
         * <p>When unset, the server returns an empty {@code <LocationConstraint>}, which S3 clients
         * universally interpret as {@code us-east-1}.
         *
         * @param region the region name, e.g. {@code "eu-west-1"}; must not be {@code null} or
         *     blank
         * @return this builder, for chaining
        ...
        public S3Forge build() {
            ...
            return new S3Forge(new S3ForgeConfig(port, virtualHostDomain, region), store);
        }
    }
}
