 ...
 *
 * <p>Routes handled (all on {@code /{bucket}/{key}}):
 *
 * <ul>
 *   <li>{@code POST ?uploads} — initiate multipart upload
 *   <li>{@code PUT ?partNumber=N&uploadId=X} — upload a part
 *   <li>{@code POST ?uploadId=X} — complete multipart upload
 *   <li>{@code DELETE ?uploadId=X} — abort multipart upload
 *   <li>{@code GET ?uploadId=X} — list parts
 * </ul>
 *
 * @since 0.1.0
 */
public final class MultipartHandler {
    ...
    public MultipartHandler(Store store) {
        if (!(store instanceof MultipartStore ms)) {
            throw new IllegalArgumentException(
                    "Store does not support multipart upload: " + store.getClass().getName());
        }
        this.store = ms;
    }
     ...
     */
    public void initiate(HttpExchange ex, String bucket, String key) throws IOException {
     ...
     *
     * <p>Stores one part and returns its ETag. If the request uses {@code aws-chunked}, the body is
     * decoded before being hashed.
     *
     * @param ex the HTTP exchange
     * @param bucket the bucket name
     * @param key the destination key
    ...
    public void uploadPart(
            HttpExchange ex, String bucket, String key, int partNumber, String uploadId)
            throws IOException {
     ...
     *
     * <p>Completes a multipart upload. The request body lists the parts to include, in ascending
     ...
     * @throws IOException on I/O failure
     */
    public void complete(HttpExchange ex, String bucket, String key, String uploadId)
            throws IOException {
        Optional<MultipartUpload> up = store.getMultipart(bucket, uploadId);
        if (up.isEmpty() || !up.get().key().equals(key)) {
            ResponseWriter.error(ex, S3Error.NO_SUCH_UPLOAD);
     ...
     *
     * <p>Aborts a multipart upload and discards all its parts.
     *
     * @param ex the HTTP exchange
     * @param bucket the bucket name
     * @param key the destination key
     ...
     * @throws IOException on I/O failure
     */
    public void abort(HttpExchange ex, String bucket, String key, String uploadId)
            throws IOException {
        Optional<MultipartUpload> up = store.getMultipart(bucket, uploadId);
        if (up.isEmpty() || !up.get().key().equals(key)) {
            ResponseWriter.error(ex, S3Error.NO_SUCH_UPLOAD);
     ...
     *
     * <p>Lists the parts uploaded so far for the given upload.
     *
     * @param ex the HTTP exchange
     * @param bucket the bucket name
     * @param key the destination key
     ...
     * @throws IOException on I/O failure
     */
    public void listParts(HttpExchange ex, String bucket, String key, String uploadId)
            throws IOException {
    ...
    /**
     * Decodes an {@code aws-chunked} body if the request carries the corresponding {@code
     * Content-Encoding}, otherwise returns the input unchanged.
     *
     * @param ex the HTTP exchange
     ...
     */
    private static byte[] decodeIfChunked(HttpExchange ex, byte[] raw) throws IOException {
     ...
     *
     * <p>Lists the in-progress multipart uploads for a bucket.
     *
     * @param ex the HTTP exchange
     * @param bucket the bucket name
     * @throws IOException on I/O failure
